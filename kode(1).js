    ...
    a = setInterval(function() {
        if (0 > r) {
            ...
            var i = getParameter("url"),
                d = base64_decode(i);
            document.getElementById("dlurl").innerHTML = '<a id="download" href="http://inisafelinku.blogspot.com/2017/06/safelink.html?url=' + i + '"><button id="btn" class="Visit_Link" style="text-align:justify;margin-bottom:2px;">Visit Link</button></a>';
            $('.adarea').remove();
        } else {
            var i = getParameter("url"),
                d = base64_decode(i);
